...
import RAW_INDEX from "public/.index/search.lunr.json";
...
const { paths: basePaths, ...rawIndex } = RAW_INDEX as unknown as RawIndex;

const baseIndex: Index = lunr.Index.load(rawIndex as Parameters<typeof lunr.Index.load>[0]);

export const initSearchIndex = (): Promise<void> => Promise.resolve();
