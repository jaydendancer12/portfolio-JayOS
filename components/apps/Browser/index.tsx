...
const Browser: FC<ComponentProcessProps> = ({ id }) => {
  ...
  const portfolioDisplayRef = useRef(false);
  ...
  const setUrl = useCallback(
    async (addressInput: string): Promise<void> => {
      ...
      if (contentWindow?.location) {
        ...
        if (lowerAddressInput.startsWith(PORTFOLIO_SITE.url)) {
          portfolioDisplayRef.current = true;
          if (inputRef.current) inputRef.current.value = "https://jays-portfolio.com";
          loadLocalSite(PORTFOLIO_SITE.path, PORTFOLIO_SITE.name);
        } else if (lowerAddressInput.startsWith(SURF_TO_MISC.url)) {
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent<unknown>): void => {
      ...
      if (
        ...
        if (msg.type === "pc-navigate" && typeof msg.url === "string") {
          goToLink(msg.url);
        }
      }
    };
      ...
      <iframe
        ...
        onLoad={() => {
          ...
          try {
            ...
            if (portfolioDisplayRef.current) {
              portfolioDisplayRef.current = false;
            } else if (newUrl && newUrl !== "about:blank" && newUrl !== currentUrl.current) {
              goToLink(newUrl);
            }
          } catch {
